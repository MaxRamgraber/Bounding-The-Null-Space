...
def build_square_grid(polygon, seed, dx):
    """
    Build a square grid inside a polygon.

    Parameters
    ----------
    polygon : list[(float, float)]
        Vertices of the polygon in world coords.
    seed : (float, float)
        World coord of the square cell that should sit at index (i=0, j=0).
    dx : float
        Square side length.
    ...
    grid : nx.Graph
        Nodes: integer ids 0..N-1 with attributes:
            - 'i', 'j'   : integer square indices
            - 'xpos','ypos' : world coordinates of cell center
            - 'vertices' : list of 4 world-coordinate vertices
            - 'area' : cell area
        Edges: between neighboring squares, with attributes:
            - 'dx' : centre-to-centre distance between the two cells
            - 'w'  : shared edge length
    """
    ...
    # Translate polygon so seed is at local origin
    poly_local = [(x - sx, y - sy) for (x, y) in polygon]
    minx = min(x for x, y in poly_local)
    maxx = max(x for x, y in poly_local)
    miny = min(y for x, y in poly_local)
    ...
    i_min = math.floor(minx / size) - 1
    i_max = math.ceil(maxx / size) + 1
    j_min = math.floor(miny / size) - 1
    j_max = math.ceil(maxy / size) + 1
    ...
    for i in range(i_min, i_max + 1):
        for j in range(j_min, j_max + 1):
            ...
            # Square vertices (clockwise)
            half = size / 2.0
            vertices = [
                (x_world - half, y_world - half),
                (x_world + half, y_world - half),
                (x_world + half, y_world + half),
                (x_world - half, y_world + half),
            ]
            ...
            ij_to_node[(i, j)] = node_id
            node_id += 1

    # 4-neighbour connectivity
    neighbor_dirs = [(1,0), (-1,0), (0,1), (0,-1)]

    center_dist = size   # centre-to-centre distance
    width = size         # shared edge length

    for (i, j), u in ij_to_node.items():
        for di, dj in neighbor_dirs:
            nbr = (i + di, j + dj)
            v = ij_to_node.get(nbr)
            if v is None or v <= u:
                continue
            grid.add_edge(u, v, dx=center_dist, w=width)
