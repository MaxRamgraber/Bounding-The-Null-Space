...
for step,T_slice_log in enumerate(np.linspace(-3,-1,101)):
    
    T_slice = 10**T_slice_log
    ...
    if __name__ == "__main__":
        ...
        class Model:
            ...
            def tighten(self, 
                ...
                checkpoint_dir = "checkpoints_pure_LP"
                ...
                pickle.dump(
                    ...
                    open(os.path.join(
                        checkpoint_dir, 
                        "iteration_initial_T_"+str(step).zfill(3)+".p"),"wb")
                    )
                ...
                for iteration in range(max_iterations_LP):
                    ...
                    if "iteration_"+str(iteration).zfill(3)+"_T_"+str(step).zfill(3)+".p" in os.listdir(checkpoint_dir):
                        
                        print("Loading past iteration")
                        
                        state = pickle.load(open(os.path.join(
                            checkpoint_dir, 
                            "iteration_"+str(iteration).zfill(3)+"_T_"+str(step).zfill(3)+".p"),"rb"))
                    ...
                    tightened_bounds = tighten_bounds(
                        ...
                        self.variables,                      # ← no z/λ
                        self.non_collapsed_variables,
                        self.bounds, self.non_collapsed_bounds,
                        verbose = "all",
                        variables_to_tighten = variables_to_tighten, # ,"T_","Sy_"
                    ...
                    for edge in self.grid.edges:
                        ...
                        if tj == ti:
                            ...
                            dhx = self.bounds[str(grid.edges[edge]["dhx"])]
                            qx = self.bounds[str(grid.edges[edge]["qx"])]
                            T = self.bounds[str(grid.edges[edge]["T"])]
                            w = self.grid.edges[edge]["w"]
                            
                            dhx, Tw, qx = contract_product(dhx, (w*T[0], w*T[1]), qx)
                            self.bounds[str(grid.edges[edge]["dhx"])] = dhx
                            self.bounds[str(grid.edges[edge]["T"])]   = (Tw[0]/w, Tw[1]/w)
                            self.bounds[str(grid.edges[edge]["qx"])]  = qx
                        ...
                        # Edge in time
                            ...
                            self.bounds[str(grid.edges[edge]["dht"])] = dht
                            self.bounds[str(grid.nodes[(ti,i)]["Sy"])]  = (Asy[0]/A, Asy[1]/A)
                            self.bounds[str(grid.edges[edge]["qt"])]  = qt
                    ...
                    pickle.dump(
                        ...
                        open(os.path.join(
                            checkpoint_dir, 
                            "iteration_"+str(iteration).zfill(3)+"_T_"+str(step).zfill(3)+".p"),"wb")
                        )
                    ...
                    plot_bounds(
                        ...
                        figure_name = os.path.join(
                            checkpoint_dir, 
                            "iteration="+str(iteration).zfill(3)+"_T_"+str(step).zfill(3)+".png"))
        ...
        for edge in list(base_grid.edges):
            base_grid.edges[edge]["flow sign"] = None
        ...
        setup_dictionary = {
            "h"     : {
                "default":      (3.0, 12.0)},
            "T"     : {
                "default":      (T_slice, T_slice)}, #  (1E-3, 1E-1)
            "R"     : {
                "default":      (0., 0.)},
            "Sy"     : {
                "default":      (0.15, 0.25)}
        ...
        for key in list(m.bounds.keys()):
            
            if key not in list(raw_bounds.keys()):
                
                raw_bounds[key] = copy.copy(m.bounds[key])
                
            else:
                
                if m.bounds[key][0] < raw_bounds[key][0]:
                    raw_bounds[key] = (m.bounds[key][0],raw_bounds[key][1])
                    
                if m.bounds[key][1] > raw_bounds[key][1]:
                    raw_bounds[key] = (raw_bounds[key][0],m.bounds[key][1])
                    
pickle.dump(
    raw_bounds,
    open("pure_LP_bounds.p","wb")
    )
