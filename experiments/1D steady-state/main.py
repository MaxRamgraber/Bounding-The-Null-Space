...
if __name__ == "__main__":
    ...
    for t in range(timesteps):
        ...
        for edge in list(base_grid.edges):
            ...
            if not ((t,j) in fixed_head_cells and (t,i) in fixed_head_cells):
                ...
                grid.edges[(t,j), (t,i)]["flow sign"] = None # -marked-
        ...
        if t > 0: # We have a transient system
            ...
            for i in nodes:
                ...
                if not ((t-1,i) in fixed_head_cells and (t,i) in fixed_head_cells):
                    ...
                    grid.edges[(t-1,i), (t,i)]["flow sign"] = None # -marked-
    ...
    # Set observations
    setup_dictionary["h"][(0,3)] = (10, 10)
    setup_dictionary["h"][(0,6)] = (7, 7)
        
    # Define source and sink cell
    setup_dictionary["R"][(0,0)] = (1E-6, 1E-4)
    setup_dictionary["R"][(0,9)] = (-1E-3,-1E-5)
    ...
    m.tighten(
        max_iterations_LP=500,
        time_limit=60.0,
        presolve=True,
        variables_to_tighten_prefixes = ["h","R","T","qx","dhx"]
    )
